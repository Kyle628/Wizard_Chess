...
function init(){
	...
	socket = io.connect('192.168.1.134:3000');
	socket.on('move', moveOpponent);
...
function moveOpponent(piece){
	console.log(piece.square);
	for(i=0; i<numPieces; i++){
		if(pieces[i].pieceId == piece.pieceId){
			pieces[i].dragging = false;
			pieces[i].moveCount++;
			pieces[i].xCoord = piece.xCoord;
			pieces[i].yCoord = piece.yCoord;
		}
	}
	drawboard();
	for (var i=0; i < 32; i++) {
